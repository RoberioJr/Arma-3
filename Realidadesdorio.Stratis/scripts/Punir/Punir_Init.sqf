...
BTC_fnc_tk_PVEH =
{
	...
	hint format ["%1 Cometeu TK e foi Punido Por %2",_name,_punisher];
...
BTC_EH_killed =
{
	...
	if (_name != name _body && BTC_vip find (name _killer) == -1) then
	{
		if (side _killer == BTC_side && _veh_body != _veh_killer) then
		{
			...
			_killer spawn 
			{
				hint format ["%1 MATOU VOCÊ! VOCÊ DECIDE SE ELE DEVE SER PUNIDO...", name _this];
				WaitUntil {Alive player};
				_action = player addAction [("<t color=""#ED2744"">") + ("Punish " + name _this) + "</t>","scripts\Punir\Punir.sqf",[name _this], 8, true, true, "", "true"];
				_timeout = time + 30;
...
BTC_Teamkill =
{
	...
	switch (true) do
	{
		...
		{
			titleText ["PARE DE ATIRAR EM SEU TIME!","BLACK FADED"];
			sleep 3;
			titleText ["PARE DE ATIRAR EM SEU TIME!","PLAIN"];
		};
		case (BTC_teamkiller > BTC_tk_blackscreen_punishment && BTC_teamkiller <= BTC_tk_last_warning) : 
		{
			...
			{
				_msg = "PARE DE ATIRAR EM SEU TIME, ULTIMO AVISO!";
			} else {_msg = "VOCÊ FOI PUNIDO POR TK!";};
			player setPos [0,0,0];
			_n = 0;
			while {_n < 60} do
			{
		...
		case (BTC_teamkiller > BTC_tk_last_warning) : 
		{
			...
			disableUserInput true;
			titleText ["VOCÊ FOI PUNIDO POR TK!\nVOCÊ NÃO È MAIS BEM VINDO","BLACK FADED"];
			player setPos [0,0,0];
			while {true} do
			{
				titleText ["VOCÊ FOI PUNIDO POR TK!\nVOCÊ NÃO È MAIS BEM VINDO","BLACK FADED"];
