...
_query = format["SELECT CASE WHEN EXISTS (SELECT pid FROM players WHERE pid = '%1') THEN 'true' ELSE 'false' END",_uid];
	...
	if(_exists) then
	{
	    _query = format["SELECT dinheiro, bancoacc, bope_gear, band_gear, bopelevel, bandidolevel, adminlevel, doadorlevel, vivo FROM players WHERE pid = '%1'",_uid];
...
_player setVariable ["RDR_Banco",_bank,true];
 ...
 //{
	[_gear,_cash,_bank,_levelbope,_levelband,_leveladmin,_leveldoador,_playervivo] remoteExecCall ["RDR_fnc_CarregarInfo",(owner _player),false];
 //};
