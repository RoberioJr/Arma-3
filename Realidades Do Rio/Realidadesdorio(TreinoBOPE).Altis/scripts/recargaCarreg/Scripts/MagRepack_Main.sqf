...
outlw_MR_populateMagListBox =
{
	...
	for "_n" from 0 to ((count _magTypes) - 1) do
	{
		...
		((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 1500) lnbSetPicture [[_n, 1], format ["scripts\recargaCarreg\Images\bulletCount\%1.paa", round((_magAmmoCounts select _n)/(_magAmmoCaps select _n)*30)]];
...
outlw_MR_onDialogDestroy =
{
	...
	if (outlw_MR_debugMode) then
	{
		...
		hint parseText ("<br/><t size='1.05'>Recarga De Carregadores</t><t size='0.95'><br/><br/>All Mags<br/>[" + str((outlw_MR_startingInfo select 0) select 0) + "] >> [" + str((_endingInfo select 0) select 0) + "]<br/>Ammo: " + _difStr + "<br/><br/>" + _output + "</t>");
	};
