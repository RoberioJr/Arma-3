   ...
   //Space key for Jumping
	...
	case 22: {
	    if (!_alt && !_ctrlKey) then {
		    ...
		    _est = locked _veh;
		    if (_veh in RJ_VeiculosDoJogador && {player distance _veh < 7}) then {
			    if (_est isEqualTo 2) then {
				    if (local _veh) then {
				            _veh lock 0;
							SystemChat "Veiculo Destrancado";
							[_veh,"PortaAbre",50,1] remoteExec ["RJ_fnc_say3D",0];
						} Else {
						    [_veh,0] remoteExecCall ["RJ_fnc_TrancarVeiculo",_veh];
							SystemChat "Veiculo Destrancado";
							[_veh,"PortaAbre",50,1] remoteExec ["RJ_fnc_say3D",0];
						};
				} Else {
				    if (local _veh) then {
						...
						[_veh,"PortaFecha",50,1] remoteExec ["RJ_fnc_say3D",0];
					} Else {
					    [_veh,2] remoteExecCall ["RJ_fnc_TrancarVeiculo",_veh];
						SystemChat "Veiculo Trancado";
						[_veh,"PortaFecha",50,1] remoteExec ["RJ_fnc_say3D",0];
					};
				};
